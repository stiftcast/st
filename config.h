...
/* st-alpha patch  -- bg opacity */
float alpha = 0.8;


/* st-anygeometry patch -- utilized by calling '-g' or '-G' on the command line */
/*
...
/* st-visualbell2 patch (both basic & enhanced -- visual indication when terminal bell is triggered. 
...
/* st-blinking_cursor patch  -- sets blinking cursor
...
/* Plumb command (right-click to plumb patch) -- select text then right click to run against selection */
static char *plumb_cmd[] = {"testscript.sh", NULL, NULL};

/* st-externalpipe-signal patch  -- runs a command using the output of ALL running st instances. 
 * Best to set a keybinding to trigger the command from your WM */
char *externalpipe_sigusr1[] = {"/bin/sh", "-c", "externalpipe_buffer.sh st_strings_read"};

/* st-externalpipe patch -- runs a command using the output of the CURRENT st instance...
 * These can be set as keybindings in the shortcuts[] struct below. */
static char *openurlcmd[] = { "/bin/sh", "-c", "st-copyout", "externalpipe", NULL };
...
/* Use Gruvbox colours by default unless loading from .Xresources */
...
static Shortcut shortcuts[] = {
    ...
    /* using 'clippaste' instead of 'selpaste' allows for pasting from global clipboard */
    ...
    /* changealpha function taken from Luke Smith's st build
     * https://github.com/LukeSmithxyz/st/commit/73a6020865607018f6442317e7f94fb5d54a7016
    */
	{ MODKEY,		        XK_w,		    changealpha,	{.f = -0.05} },
	{ MODKEY,		        XK_m,		    changealpha,	{.f = +0.05} },
